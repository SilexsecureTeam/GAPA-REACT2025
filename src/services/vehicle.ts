...
export function vehicleMatches(product: any, state: VehicleFilterState): boolean {
  ...
  if (Array.isArray(suitability) && suitability.length > 0) {
    ...
    return suitability.some((entry: any) => {
      ...
      // If no sub_suitability_models array but we matched Brand, 
      // check loosely if top-level model name matches user selection?
      // Usually strict filtering requires sub-models. 
      // But if data is sparse, we fallback to false here and let the legacy check run? 
      // No, usually if suitability_models exists, it's the source of truth.
      return false
    })
  }
