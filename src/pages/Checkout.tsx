...
import { getCartForUser, removeCartItem, addToCartApi, getProductById, getAllStatesApi, getStatesByLocation, updateDeliveryAddress, /* getUserCartTotal, */ getPriceByState, paymentSuccessfull, getGigQuote, type GigQuoteParams, getUserProfileById, updateUserProfile } from '../services/api'
...
export default function Checkout() {
  ...
  const { user } = useAuth()
  // Updated hook destructuring to include setCurrency
  ...
  useEffect(() => {
    ...
    // Load profile explicitly to get phone/address if not in context
    if (user?.id) {
       getUserProfileById(user.id).then(u => {
          if (u) {
             setAddress(prev => ({
                ...
                // If user has region/state, we might match it later in the hydration effect
  ...
  const handleContinueFromAddress = async () => {
    ...
    if (user && (user as any).id) {
      ...
      try { 
         // Update both generic address and structured profile
