...
  }, [vehFilter])
  ...
  const ManufacturerFilterBar = ({ sourceProducts }: { sourceProducts: ApiProduct[] }) => {
    // 1. Find manufacturers relevant to the current product list
...
 // 1. Base list: Filters applied BEFORE manufacturer (used to generate filter options)
  ...
  // 2. Final list: Applies manufacturer filter (used for grid display)
...
  }, [pageSearch])
  ...
  // Apply Manufacturer Filter to the main filtered list
...
  // Update this existing hook to use 'manufacturerFiltered' instead of 'filtered'
  const displayFiltered = useMemo(() => {
    if (!pageSearch || !pageSearch.trim()) return manufacturerFiltered // Changed from 'filtered'
    return manufacturerFiltered.filter(p => matchesPageSearch(p)) // Changed from 'filtered'
...
  // Derived values for drill-down (must not be inside conditionals to respect Hooks rules)
  // Base list for sub-category view
  ...
  // Final list with manufacturer filter
