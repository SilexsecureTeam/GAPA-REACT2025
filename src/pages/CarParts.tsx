          ...
          {(brands.length ? brands : []).slice(0, 12).map((b, i) => {
            ...
            return (
              <div key={key} className="shrink-0">
                {logo ? <img src={normalizeApiImage(logo) || logoImg} alt={name} className="h-12 w-auto object-contain" onError={(e)=>{(e.currentTarget as HTMLImageElement).src='/gapa-logo.png'}} /> : <span className="text-[13px] font-medium">{name}</span>}
              </div>
            )
          })}
        </div>
